...
====
[%metadata]
identifier:: /req/geopackage-3d-models/req...
description:: For defining ...
part:: A 3D model SHALL be defined as an entry in a `...` table with the following columns
...
See an https://portal.ogc.org/files/?artifact_id=95351[example GeoPackage] using reference points as Mapbox Vector Tiles and glTF models

See an https://portal.ogc.org/files/?artifact_id=95340[example GeoPackage] using reference points as GNOSIS Map Tiles and E3D models
...
See an https://portal.ogc.org/files/?artifact_id=95337[example GeoPackage] using batched 3D models encoded as glTF

See an https://portal.ogc.org/files/?artifact_id=95334[example GeoPackage] using batched 3D models encoded as E3D
...
See also the https://github.com/sofwerx/cdb2-eng-report/blob/master/11-tiling-coverages.adoc#tiled-3d-models[CDB X Discussion Paper] describing the experiments in which this extension was developed, along with the resulting sample GeoPackages.
